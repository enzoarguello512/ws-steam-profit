...
def link_sanitizer(link):
    """Basicamente elimina la ultima parte del link de steam que por lo general son numeros random y lo
    junta todo en un string"""
    new_link = []
...
class SteamGameFinder(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if page_exists is not None: # Osea , si hay texto en la pagina es porque no hay juegos, y eso significaria que
            # la pagina no existe
