...
LFullAppidLinkPLUS = df["Steamcardexchange link"]
LGamePricePLUS = list(df["Game price (ARS)"])   #lo convertimos a lista para poder manejarlo mejor con python porque sino creo que te da un dataframe objetc
ua = UserAgent ()
amountPLUS = 0
...
class CardsSpider(scrapy.Spider):
    ...
    custom_settings = {
        'DOWNLOAD_DELAY': 2.0,  #Lo ideal para mi seria entre 2 o 3
        'CONCURRENT_REQUESTS': 1,
        'USER_AGENT': str(ua.random),   #Para evitar un posible Ban de 'navegador' , pero no evitariamos uno de ip , para eso esta TOR browser
