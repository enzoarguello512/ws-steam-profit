...
LOfferType1 = list(df1["Offer type"].dropna())
# LOfferType1.remove()
print(LOfferType1)
# LOfferType = LOfferType + LOfferType1
LOfferType = LOfferType1 + LOfferType

# LOfferType.reverse()
print(LOfferType)
LNewHighestDiscount = list(df1["New highest discount?"]) + LNewHighestDiscount
LDaysSinceTheOfferStarted = list(df1["Days since the offer started"]) + LDaysSinceTheOfferStarted
LDaysForTheOfferToEnd = list(df1["Days for the offer to end"]) + LDaysForTheOfferToEnd
